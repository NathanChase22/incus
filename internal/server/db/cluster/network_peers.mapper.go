...
func GetNetworkPeers(ctx context.Context, db dbtx, filters ...NetworkPeerFilter) (_ []NetworkPeer, _err error) {
	...
	for i, filter := range filters {
		...
		} else if filter.Name != nil && filter.ID == nil && filter.NetworkID == nil {
...
func GetNetworkPeer(ctx context.Context, db dbtx, name string) (_ *NetworkPeer, _err error) {
	defer func() {
		_err = mapErr(_err, "Network_peer")
	}()
...
func GetNetworkPeerID(ctx context.Context, db tx, name string) (_ int64, _err error) {
	defer func() {
		_err = mapErr(_err, "Network_peer")
	}()
...
func UpdateNetworkPeer(ctx context.Context, db tx, name string, object NetworkPeer) (_err error) {
	defer func() {
		_err = mapErr(_err, "Network_peer")
	}()

	id, err := GetNetworkPeerID(ctx, db, name)
	if err != nil {
		return err
	}
